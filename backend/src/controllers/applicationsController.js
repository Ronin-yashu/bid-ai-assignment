...
export const submitApplication = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Application submitted successfully! We will get back to you soon.', application: result.rows[0] })
  } catch (err) { next(err) }
}
...
export const getApplications = async (req, res, next) => {
  try {
    ...
    const query = `SELECT a.*, j.title as job_title, j.department FROM applications a LEFT JOIN jobs j ON a.job_id = j.id ${where} ORDER BY a.created_at DESC LIMIT $${params.length+1} OFFSET $${params.length+2}`
